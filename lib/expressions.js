...
internals.cleanKey = uncleandKey => _.chain(uncleandKey)
  .map(i => (
    (i >= 'A' && i <= 'Z') || (i >= 'a' && i <= 'z') ?
    i :
    _.padStart(i.charCodeAt(0).toString(16), 4, '0')
  ))
  .join('')
  .value();
...
internals.uniqAttributeValueName = (key, existingValueNames) => {
  const cleanedKey = internals.cleanKey(key).replace(/002e/g, '_');
...
exports.buildFilterExpression = (key, operator, existingValueNames, val1, val2) => {
  ...
  const path = `#${keys.map(internals.cleanKey).join('.#')}`;
  ...
  keys.forEach((key) => {
    attributeNames[`#${internals.cleanKey(key)}`] = key;
